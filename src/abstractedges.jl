...
Base.getindex(p::AbstractEdges, I) = Tuple(p[i] for i ∈ I)#[p[i] for i ∈ I]
Base.getindex(p::AbstractEdges, ::Colon) = p.ftrs
Base.firstindex(p::AbstractEdges) = 1
Base.lastindex(p::AbstractEdges) = p.N
# Base.setindex!(p::AbstractEdges, v, i) = setindex!(p.ftrs, v, i)
...
# abstract type AbstractEdges2{U, N} end
# struct Edges2{U<:Tuple{Vararg{T, N} where {T<:AbstractEdge}} where {N}, N} <: AbstractEdges2{U, N}
#     ftrs::U
# end
# Edges2(ftrs::U) where {U<:Tuple{Vararg{T, N} where {T<:AbstractEdge}}} where {N} =
#     Edges2{U,N}(ftrs)

# Base.length(p::AbstractEdges2{U, N}) where {U, N} = N

# Base.iterate(p::AbstractEdges2{U, N}, state=1) where {U, N} = state > N ? nothing : (p.ftrs[state], state + 1)
# Base.eltype(p::AbstractEdges2) = eltype(p.ftrs)

# Base.IndexStyle(::Type{<:AbstractEdges2}) = IndexLinear()
# Base.getindex(p::AbstractEdges2, i::Int) = getindex(p.ftrs, i)
# Base.getindex(p::AbstractEdges2, I) = Tuple(p[i] for i ∈ I)#[p[i] for i ∈ I]
# Base.getindex(p::AbstractEdges2, ::Colon) = p.ftrs
# Base.firstindex(p::AbstractEdges2) = 1
# Base.lastindex(p::AbstractEdges2{U, N}) where {U, N} = N
# # Base.setindex!(p::AbstractEdges2, v, i) = setindex!(p.ftrs, v, i)

# function Base.isequal(p₁::AbstractEdges2, p₂::AbstractEdges2)
#     p₁ === p₂ && return true
#     length(p₁) == length(p₂) || return false
#     for n = 1:length(p₁)
#         p₁[n] == p₂[n] || return false
...
#     return true
# end
# Base.:(==)(p₁::AbstractEdges2, p₂::AbstractEdges2) = isequal(p₁, p₂)

# #### Outer constructors: Edges2
# Edges2(gen::T) where {T<:Base.Generator} = Edges2(Tuple(gen))

# #### functor: Edges
# @generated function (p::AbstractEdges2{U, N})(x, A) where {U, N}
#     quote
...
# function (p::AbstractEdges2{U, N})(A) where {U, N}
